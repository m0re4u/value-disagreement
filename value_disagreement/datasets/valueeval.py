...
class ValueEvalDataset(ValueDataset):
    def __init__(self, data_dir, focus_value=None, cast_to_valuenet=False, return_predefined_splits=False):
        """
        ...
        self.arguments['raw_labels'] = [self.label_df.columns[i].tolist() for i in self.label_df.values == np.array(self.label_df.max(axis=1))[:, None]]
        ...
        self.arguments['text'] = self.arguments[['Conclusion', 'Premise']].apply(self.remap_text, axis=1)
        self.arguments['schwartz_labels'] = self.arguments['raw_labels'].apply(self.remap_labels)
    ...
    def reformat_label(self):
        ...
        """
        self.arguments['focus_value_present'] = self.arguments['schwartz_labels'].apply(lambda x: int(self.focus_value in x))
    ...
    def cast_to_valuenet(self):
        ...
        self.arguments = pd.DataFrame.from_records(all_samples, columns=['text', 'orig_label', 'value', 'split'])
    ...
    def get_splits(self, train_size=0.8, val_size=0.1, test_size=0.1):
        ...
        if self.return_predefined_splits:
            ...
            load_splits_path = Path(f"{self.dataset_dir}/../../valueeval/splits.npy")
            if load_splits_path.exists():
                test_set_index = np.load(load_splits_path, allow_pickle=True)
            else:
                samples_per_val = np.round(num_test * np.random.dirichlet(np.ones(len(ValueConstants.SCHWARTZ_VALUES)), size=1))
            ...
            val_set = self.arguments[~self.arguments.index.isin(test_set_index) & ~self.arguments.index.isin(train_set.index)]
    ...
    def __getitem__(self, idx):
        ...
        if self.focus_value is not None:
            return self.arguments.iloc[idx]['text'], self.arguments.iloc[idx]['focus_value_present']
        else:
