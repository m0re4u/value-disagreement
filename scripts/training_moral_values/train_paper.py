...
def get_dataset(args):
    ...
    if args.train_dataset == "both":
        ...
        dataset_eval = ValueEvalDataset(
            f"{args.project_dir}/data/valueeval/dataset-identifying-the-human-values-behind-arguments/",
        ...
        if args.train_dataset == "valuenet":
            ...
            dataset = RedditAnnotatedDataset("data/annotated_data/climatesingle_small.csv")
        elif args.train_dataset == "climatesingle-large":
            dataset = RedditAnnotatedDataset("data/annotated_data/climatesingle_large.csv")
...
def main(args):
    ...
    # Tokenize data
    tokenized_dataset_train = hf_dataset_tokenize(train_hf, value_tokenizer, args.use_model)
    tokenized_dataset_val = hf_dataset_tokenize(val_hf, value_tokenizer, args.use_model)
    tokenized_dataset_test = hf_dataset_tokenize(test_hf, value_tokenizer, args.use_model)
    ...
    training_args = TrainingArguments(
        f"value_estimation_trainer_{args.train_dataset}",
    ...
    if args.predict_only:
        ...
        dump_predictions(tokenized_dataset_test, results, f"output/predictions/moral_values_{args.train_dataset}.json")
    elif args.eval_only:
        print(trainer.evaluate(tokenized_dataset_test))
    else:
        # Run training
        if args.ray:
            def hyper_space(x): return {
...
if __name__ == "__main__":
    ...
    parser.add_argument('train_dataset', choices=["valuenet", "valueeval", "climatesingle-small", "climatesingle-large", "both"],
    ...
    parser.add_argument('--n_runs', type=int, default=1,
                        help="Number of times to train a model")
    ...
    parser.add_argument('--no_label_remap', action='store_false', default=True,
